...
handle-mouse-event() {
  ...
  local mouse_CURSOR
  echo my: $my Y: $Y cy: $cy >> ~/mouse.log
  echo 'my + Y - cy = ' $(( my + Y - cy )) >> ~/mouse.log
  echo $pos >> ~/mouse.log
  for key val in "${(@kv)pos}"; do
    echo -n "$key -> $val  " >> ~/mouse.log
  done
  echo
  echo '=======================' >> ~/mouse.log
  if ((my + Y - cy > y)); then
...
handle-xterm-mouse-event() {
  ...
  echo button: $bt x: $mx y: $my l-s: $last_status >> ~/mouse.log
